// === Rule34 Integration Module (r34-bot.js) ===
// Commands: /r34, /furina, /expr, /switch, /avatarchange, /fav, /help
...
(function () {
  ...
  oc.thread.on("MessageAdded", async function ({ message }) {
    ...
    const commandPrefixes = ["/r34", "/furina", "/expr", "/switch", "/avatarchange", "/fav", "/help"];
    ...
    if (!R34_API_KEY || R34_API_KEY === "YOUR_API_KEY_HERE") {
      oc.thread.messages.push({
        author: "system",
        content: "⚠️ Please set your valid Rule34 API key in the custom module first.",
        expectsReply: false,
        hiddenFrom: ["ai"]
      });
      return;
...
\`\/avatarchange on|off\` — Toggle dynamic avatar updates  
      ...
      const response = await fetch(`https://api.rule34.xxx/index.php?${params}`, {
        headers: { "User-Agent": "PerchanceBot/3.0" }
      });
